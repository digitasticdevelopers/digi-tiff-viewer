...
UTIF.decode._decompress = function(img,ifds, data, off, len, cmpr, tgt, toff, fo)  // fill order
{
	...
	//var time = Date.now();
	if(false) {}
	else if(cmpr==1 || len==tgt.length) for(var j=0; j<len; j++) tgt[toff+j] = data[off+j];
...
UTIF._readIFD = function(bin, data, offset, ifds, depth, debug)
{
	...
	for(var i=0; i<cnt; i++)
	{
		...
		if(tag==330 || tag==34665) {
			var subfd = [];
			for(var j=0; j<num; j++) UTIF._readIFD(bin, data, arr[j], subfd, depth+1, debug);
...
UTIF.replaceIMG = function()
{
  console.log('replaceImg')
	var imgs = document.getElementsByClassName("tiff");
...
UTIF._imgLoaded = function(e)
{
  ...
  var canvasTiff  = document.getElementsByClassName("canvasTiff")
  UTIF.decodeImage(buff, page, ifds);
	var rgba = UTIF.toRGBA8(page), w=page.width, h=page.height;
	var ind = UTIF._xhrs.indexOf(e.target), img = UTIF._imgs[ind];
	UTIF._xhrs.splice(ind,1);  UTIF._imgs.splice(ind,1);
	var cnv = document.getElementById("drawImage");  cnv.width=w;  cnv.height=h;
  ...
  cnv.setAttribute('id','drawImage')
  img.parentNode.appendChild(cnv, img);
}
